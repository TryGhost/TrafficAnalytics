...
Both modes run from the same image; the role is selected by the `WORKER_MODE` environment variable (worker vs. ingest). Page hits use `PUBSUB_TOPIC_PAGE_HITS_RAW` and Tinybird sync events use `PUBSUB_TOPIC_TINYBIRD_SYNC` to select batch vs. inline handling independently. See [docs/architecture.md](docs/architecture.md) for a diagram and full detail.
