...
- **Ingest app** (`WORKER_MODE` unset — `src/app.ts`): the Fastify HTTP server that receives page-hit and Tinybird sync events.
- **Worker app** (`WORKER_MODE=true` — `src/worker-app.ts`): consumes page-hit and Tinybird sync subscriptions, batches events, and forwards them to Tinybird. Exposes only health endpoints (`/`, `/health`).
...
The Tinybird sync endpoint chooses its strategy independently using `PUBSUB_TOPIC_TINYBIRD_SYNC`: when set, validated events are published to Pub/Sub and the worker batches each event type separately; when unset, they are sent directly to Tinybird.
...
- **Routes / handlers** (`src/routes/v1`, `src/handlers/`): defines the page-hit and Tinybird sync endpoints and chooses their batch vs inline strategies.
- **Plugins** (`src/plugins/`): `hmac-validation` (global `preValidation` HMAC check), `bot-detection` (page-hit `preHandler` bot filter), `timestamp` (records `serverReceivedAt` on request), `cors`, `logging`, `proxy` (local `/local-proxy` test endpoint), and the page-hit/Tinybird sync worker lifecycle plugins.
- **Events** (`src/services/events/`): `publisher.ts` / `publisherUtils.ts` publish raw page hits and Tinybird sync events to Pub/Sub; `subscriber.ts` consumes from a subscription. Uses `@google-cloud/pubsub`.
...
- **Tinybird sync worker** (`src/services/tinybird-sync-worker/`): validates Tinybird sync messages, maintains separate `automation_runs` and `automation_run_steps` batches, and flushes each type to its mapped Tinybird datasource.
...
Tinybird sync batch mode follows the same publish/consume pattern through `PUBSUB_TOPIC_TINYBIRD_SYNC` and `PUBSUB_SUBSCRIPTION_TINYBIRD_SYNC`. The worker validates each message, routes it by `type`, and independently flushes `automation_runs` to `automation_run_events` and `automation_run_steps` to `automation_run_step_events`.
...
- `PUBSUB_TOPIC_TINYBIRD_SYNC` - Topic the ingest app publishes validated Tinybird sync events to. If set, Tinybird sync uses batch mode; if unset, events are sent to Tinybird inline.
- `PUBSUB_SUBSCRIPTION_TINYBIRD_SYNC` - Subscription consumed by the Tinybird sync batch worker.
...
- `BATCH_SIZE` - Worker flush batch size (default: 50); Tinybird sync workers apply it independently per event type
