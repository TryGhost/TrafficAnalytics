...
Traffic Analytics deploys to **Google Cloud Run** in a single region (`europe-west4`, "netherlands"), across **staging** and **production** environments. Each environment runs three Cloud Run services: ingest, page-hit worker, and Tinybird sync worker.
...
flowchart TD
    ...
    Build --> Stg["deploy-environment.yml<br/>staging (analytics + worker + tinybird-sync)"]
    Build --> Prd["deploy-environment.yml<br/>production (analytics + worker + tinybird-sync)"]
...
4. **Deploy Staging** and **Deploy Production** — both call [`deploy-environment.yml`](../.github/workflows/deploy-environment.yml) after the build, so the two environments deploy in parallel. Each deploys `analytics`, `worker`, and `tinybird-sync`.
...
```text
<stg|prd>-<region_name>-traffic-analytics[-worker|-tinybird-sync-worker]
```
...
2. Builds and pushes the image, then deploys `analytics`, `worker`, and `tinybird-sync` to staging.
