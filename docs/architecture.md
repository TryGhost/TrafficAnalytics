...
- The reported service name is `analytics-worker` in batch worker mode, `analytics-tinybird-sync-worker` in Tinybird sync mode, and `analytics-service` otherwise.
